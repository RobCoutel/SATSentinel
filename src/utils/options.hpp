 ...
 *
 * @brief Generic, self-documenting command-line option system, mirroring NapSAT's own
 * src/utils/options.hpp (duplicated rather than shared, since SATSentinel must build standalone
 * without depending on NapSAT). An Option binds by reference to an existing field (bool/int/string),
 * knows its CLI aliases, and can declare fixed-priority incompatibilities with other options
 * (subsumes) and simple AND-requirements on other options (require). OptionParser collects a set of
 * Options, parses a token stream into them, resolves incompatibilities/requirements, and can print a
 * help text describing every registered option.
 */
#pragma once
...
namespace sentinel
{
  ...
  public:
    IntOption(std::string name, int& storage, std::string brief, std::string category);
    bool consume(const std::vector<std::string>& tokens, unsigned& i) override;
    void reset_to_default() override { _storage = _default; }
    ...
    int value() const { return _storage; }
  ...
  public:
    ...
    IntOption& add_int(const std::string& name, int& storage, const std::string& brief);
    ...
    /** @brief Runs subsumption and requirement rules to a fixpoint. */
    void resolve();

    /** @brief Full help text for every registered option, grouped by category. */
    std::string help_text(const std::string& title, const std::string& header = "") const;
